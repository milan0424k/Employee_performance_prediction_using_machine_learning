...
def predict():
    ...
    if prediction <= 0.40:
     level = "LOW"
     text = f"Predicted Productivity Score: {prediction:.2f} : The employee has LOW productivity"
    elif prediction <= 0.70:
     level = "MEDIUM"
     text = f"Predicted Productivity Score: {prediction:.2f} : The employee has MEDIUM productivity"
    else:
     level = "HIGH"
     text = f"Predicted Productivity Score: {prediction:.2f} : The employee has HIGH productivity"
    ...
    return render_template('submit.html', prediction_text=text, level=level,graphs=graphs)
